...
BEGIN
  ...
  IF Length(entry) = 0 THEN
    WriteString("mxpkg: no entry module specified in manifest"); WriteLn;
    RAISE BuildError
  END;
  ...
  WriteString("mxpkg: "); WriteString(cmd); WriteLn;
  rc := m2sys_exec(ADR(cmd));
  IF rc # 0 THEN
    WriteString("mxpkg: build failed (exit "); WriteInt(rc, 1); WriteString(")"); WriteLn;
    RAISE BuildError
  END;
  WriteString("mxpkg: built "); WriteString(outPath); WriteLn
END Build;
...
BEGIN
  ...
  WriteString("mxpkg: running "); WriteString(runCmd); WriteLn;
