...
BEGIN
  ...
  m2sys_join_path(ADR(home), ADR(".mxpkg"), ADR(t), 1024);
...
BEGIN
  ...
  IF rc # 0 THEN
    WriteString("mxpkg: failed to create registry directory"); WriteLn;
    RAISE RegistryError
  END;
  m2sys_join_path(ADR(regDir), ADR("index"), ADR(idxDir), 1024);
  rc := m2sys_mkdir_p(ADR(idxDir));
  IF rc # 0 THEN
    WriteString("mxpkg: failed to create registry index directory"); WriteLn;
    RAISE RegistryError
  END;
  m2sys_join_path(ADR(regDir), ADR("packages"), ADR(pkgDir), 1024);
  rc := m2sys_mkdir_p(ADR(pkgDir));
  IF rc # 0 THEN
    WriteString("mxpkg: failed to create registry packages directory"); WriteLn;
    RAISE RegistryError
  END;
  GetCacheDir(cacheDir);
  rc := m2sys_mkdir_p(ADR(cacheDir));
  IF rc # 0 THEN
    WriteString("mxpkg: failed to create cache directory"); WriteLn;
    RAISE RegistryError
  END
END Init;
...
BEGIN
  ...
  IF Length(name) = 0 THEN
    WriteString("mxpkg: no package name in manifest"); WriteLn;
    RAISE RegistryError
  END;
  IF Length(ver) = 0 THEN
    WriteString("mxpkg: no version in manifest"); WriteLn;
    RAISE RegistryError
  END;

  (* Semver enforcement *)
  IF IsValid(ver) = 0 THEN
    WriteString("mxpkg: version '"); WriteString(ver);
  ...
  IF rc # 0 THEN
    WriteString("mxpkg: failed to create tarball"); WriteLn;
    RAISE RegistryError
  END;
  ...
  IF rc # 0 THEN
    WriteString("mxpkg: failed to hash tarball"); WriteLn;
    RAISE RegistryError
  END;
  ...
  IF fh < 0 THEN
    WriteString("mxpkg: failed to write index entry"); WriteLn;
    RAISE RegistryError
  END;
  rc := m2sys_flock(fh, 1);
  TRY
  ...
  WriteString("mxpkg: published "); WriteString(name);
...
BEGIN
  ...
  IF m2sys_file_exists(ADR(tarPath)) = 0 THEN
    WriteString("mxpkg: tarball not found: "); WriteString(tarPath); WriteLn;
    RAISE RegistryError
  END;
  ...
  IF CompareStr(sha, actualSha) # 0 THEN
    WriteString("mxpkg: sha256 mismatch for "); WriteString(name);
  ...
  IF rc # 0 THEN
    WriteString("mxpkg: failed to extract tarball"); WriteLn;
    RAISE RegistryError
  END;

  Assign(cachePkg, pathBuf);
  WriteString("mxpkg: fetched "); WriteString(name);
...
BEGIN
  ...
  IF st # HTTPClient.OK THEN
    WriteString("mxpkg: failed to connect to "); WriteString(url); WriteLn;
    RAISE RegistryError
  END;
  ...
  IF Length(sha) = 0 THEN
    WriteString("mxpkg: no sha256 in remote index for "); WriteString(name); WriteLn;
    RAISE RegistryError
  END;
  ...
  rc := m2sys_sha256_file(ADR(tarPath), ADR(actualSha));
  IF rc # 0 THEN
    WriteString("mxpkg: failed to hash remote tarball"); WriteLn;
    RAISE RegistryError
  END;
  IF CompareStr(sha, actualSha) # 0 THEN
    WriteString("mxpkg: sha256 mismatch for remote "); WriteString(name);
    WriteString(" "); WriteString(version); WriteLn;
    RAISE RegistryError
  END;
  ...
  rc := m2sys_tar_extract(ADR(tarPath), ADR(cachePkg));
  IF rc # 0 THEN
    WriteString("mxpkg: failed to extract remote tarball"); WriteLn;
    RAISE RegistryError
  END;

  Assign(cachePkg, pathBuf);
  WriteString("mxpkg: fetched remote "); WriteString(name);
  WriteString(" "); WriteString(version);
  WriteString(" -> "); WriteString(cachePkg); WriteLn
END FetchRemote;
...
BEGIN
  IF httpStatus = 404 THEN
    WriteString("mxpkg: package not found on remote registry: ");
    WriteString(name); WriteString(" "); WriteString(version); WriteLn
  ELSIF httpStatus = 401 THEN
    WriteString("mxpkg: authentication required (set MXPKG_TOKEN)"); WriteLn
  ELSIF httpStatus = 409 THEN
    WriteString("mxpkg: version already exists: ");
    WriteString(name); WriteString(" "); WriteString(version); WriteLn
  ELSIF httpStatus = 413 THEN
    WriteString("mxpkg: package exceeds max size"); WriteLn
  ELSIF httpStatus >= 500 THEN
    WriteString("mxpkg: remote registry error (HTTP ");
    WriteInt(httpStatus, 1); WriteString(")"); WriteLn
  ELSE
    WriteString("mxpkg: remote registry HTTP error ");
...
BEGIN
  ...
  IF Length(token) = 0 THEN
    WriteString("mxpkg: no auth token (set MXPKG_TOKEN)"); WriteLn;
    RAISE RegistryError
  END;
  ...
  IF fileSize64 <= 0 THEN
    WriteString("mxpkg: tarball is empty or unreadable"); WriteLn;
    RAISE RegistryError
  END;
  fileSize := INTEGER(fileSize64);
  ALLOCATE(bodyPtr, fileSize);
  IF bodyPtr = NIL THEN
    WriteString("mxpkg: out of memory reading tarball"); WriteLn;
    RAISE RegistryError
  END;
  Assign("r", rmode);
  ...
  IF fh < 0 THEN
    ...
    WriteString("mxpkg: failed to open tarball"); WriteLn;
    RAISE RegistryError
  END;
  ...
  IF Auth.QuickSignHS256(token, "mxpkg", 300, jwt, jwtLen) # Auth.OK THEN
    WriteString("mxpkg: invalid auth token (expected 64-char hex secret)"); WriteLn;
    DEALLOCATE(bodyPtr, fileSize);
    RAISE RegistryError
  END;
  ...
  IF st # HTTPClient.OK THEN
    WriteString("mxpkg: publish failed — could not connect to registry"); WriteLn;
...
BEGIN
  ...
  st := SyncGet(apiUrl, mInsecure, resp);
  IF st # HTTPClient.OK THEN
    WriteString("mxpkg: search failed — could not connect to registry"); WriteLn;
    RAISE RegistryError
  END;
  IF resp = NIL THEN
    WriteString("mxpkg: search failed — no response"); WriteLn;
    RAISE RegistryError
  END;
  ...
  WriteString("mxpkg: search results for '"); WriteString(query);
  WriteString("':"); WriteLn;
